...
/* `#END` */
    ...
    *******************************************************************************/
    CY_ISR(UART_test_RXISR)
    {
    ...
    #ifdef UART_test_RXISR_ENTRY_CALLBACK
        UART_test_RXISR_EntryCallback();
    #endif /* UART_test_RXISR_ENTRY_CALLBACK */
        ...
        do
        {
            ...
            readStatus = UART_test_RXSTATUS_REG;
            /* Copy the same status to readData variable for backward compatibility support 
            *  of the user code in UART_test_RXISR_ERROR` section. 
            */
            readData = readStatus;

            if((readStatus & (UART_test_RX_STS_BREAK | 
                            UART_test_RX_STS_PAR_ERROR |
                            UART_test_RX_STS_STOP_ERROR | 
                            UART_test_RX_STS_OVERRUN)) != 0u)
            {
                /* ERROR handling. */
                UART_test_errorStatus |= readStatus & ( UART_test_RX_STS_BREAK | 
                                                            UART_test_RX_STS_PAR_ERROR | 
                                                            UART_test_RX_STS_STOP_ERROR | 
                                                            UART_test_RX_STS_OVERRUN);
                /* `#START UART_test_RXISR_ERROR` */

                /* `#END` */
                
            #ifdef UART_test_RXISR_ERROR_CALLBACK
                UART_test_RXISR_ERROR_Callback();
            #endif /* UART_test_RXISR_ERROR_CALLBACK */
            }
            
            if((readStatus & UART_test_RX_STS_FIFO_NOTEMPTY) != 0u)
            {
                ...
                {
                    if((readStatus & UART_test_RX_STS_MRKSPC) != 0u)
                    {
                        if ((readStatus & UART_test_RX_STS_ADDR_MATCH) != 0u)
                        {
                ...
                if(increment_pointer != 0u)
                {
                    ...
                    if(UART_test_rxBufferWrite >= UART_test_RX_BUFFER_SIZE)
                    {
                    ...
                    if(UART_test_rxBufferWrite == UART_test_rxBufferRead)
                    {
                        ...
                        #endif /* (UART_test_FLOW_CONTROL != 0u) */
                    }
                }
            }
        }while((readStatus & UART_test_RX_STS_FIFO_NOTEMPTY) != 0u);
    ...
    #ifdef UART_test_RXISR_EXIT_CALLBACK
        UART_test_RXISR_ExitCallback();
    #endif /* UART_test_RXISR_EXIT_CALLBACK */
    ...
    *******************************************************************************/
    CY_ISR(UART_test_TXISR)
    {
    ...
    #ifdef UART_test_TXISR_ENTRY_CALLBACK
        UART_test_TXISR_EntryCallback();
    #endif /* UART_test_TXISR_ENTRY_CALLBACK */
             ...
             ((UART_test_TXSTATUS_REG & UART_test_TX_STS_FIFO_FULL) == 0u))
        {
            ...
            if(UART_test_txBufferRead >= UART_test_TX_BUFFER_SIZE)
            {
    ...
    #ifdef UART_test_TXISR_EXIT_CALLBACK
        UART_test_TXISR_ExitCallback();
    #endif /* UART_test_TXISR_EXIT_CALLBACK */
