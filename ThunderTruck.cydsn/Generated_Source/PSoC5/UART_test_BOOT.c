         ...
         
{
    ...
    while(bufIndex < size)
    {
        UART_test_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        *  UART_test_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(UART_test_BL_CHK_DELAY_MS);
        }
    }
    ...
    while(UART_test_GetRxBufferSize() > 0u)
    {
        ...
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            ...
            *  If not wait for additional UART_test_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != UART_test_PACKET_EOP)
            {
                CyDelay(UART_test_WAIT_EOP_DELAY);
            }
        }
